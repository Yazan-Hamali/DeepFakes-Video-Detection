...
from dataset.mydata import MyDataset
...
def main():
	...
	model = TransferModel(modelchoice='xception',num_out_classes=2,pre=True)
    # model = TransferModel(modelchoice='xception',num_out_classes=2,pre=False)
	if continue_train:
	...
	for epoch in range(epoches):
		...
		for (image, labels) in train_loader:
			...
			train_loss += iter_loss  # update running training loss
	...
	torch.save(model.module.state_dict(), os.path.join(output_path, "best.p"))
...
if __name__ == '__main__':
	...
	parse.add_argument('--model_name', '-mn', type=str, default='fs_c0_299.p')
	parse.add_argument('--continue_train', type=bool, default=False)
	parse.add_argument('--model_path', '-mp', type=str, default='./output/df_xception_c0_299/1_df_c0_299.p')
	main()